...
class AIPlayer(Player):
    ...
    def getMove(self, currentState):
        print(self.heuristicStepsToGoal(currentState.fastclone()))
    ...
    def heuristicStepsToGoal(self, currentState):
      ...
      myOffense = getAntList(currentState, me, (SOLDIER, R_SOLDIER, DRONE))
      shortest = 1000
        ...
        #Otherwise, we want to move toward the food
          ...
          for i in range(len(distanceToFood)):
            if distanceToFood[i] < min_food:
              ...
              idx = i
          distanceToTunnel = stepsToReach(currentState, foods[i].coords, myTunnel.coords)
          distanceToHill = stepsToReach(currentState, foods[i].coords, myHill.coords)
          min_dist = min(distanceToTunnel, distanceToHill) + min_food
          food_times.append(min_dist)
      
      foodDists = []
      for food in foods:
        distanceToTunnel = stepsToReach(currentState, food.coords, myTunnel.coords)
        distanceToHill = stepsToReach(currentState, food.coords, myHill.coords)
